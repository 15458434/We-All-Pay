...
class ExchangeRateFetcherTest: XCTestCase {
    ...
    func testIsLastFetchOlderThanAnHour() {
        ...
        let expectation = self.expectation(withDescription: "isLastFetchOlderThanHour")
        ...
        waitForExpectations(withTimeout: 90, handler: { (error) -> Void in
            XCTAssertNil(error, "Timeout error: \(error)")
        })
    }

    func testFetchFromOpenExchangeRates() {
        let expectation = self.expectation(withDescription: "fetchFromOpenExchangeRates")
    ...
    func testExchangeRate() {
        let expectation = self.expectation(withDescription: "exchangeRate")
    ...
    func testCodeSubscript() {
        let expectation = self.expectation(withDescription: "codeSubscript")
